...
declare -a file_base=("Blood-Distribution/s4.1" "Blood-Distribution/s4.2" "Blood-Distribution/s4.3" 
                        "Blood-Distribution/s4.4" "Blood-Distribution/s4.5")
...
for file in "${file_base[@]}"; do
    ...
    python3.6 $bin $file_in > "$file_out_tst"

    # Execute diff
    $diff --strip-trailing-cr "$file_out_tst" "$file_out_val"
